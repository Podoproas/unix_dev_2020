...
#define buf_grow(v, n) \
    ((v) = buf_grow1((v), sizeof(*(v)), n, BUF_ABORT))

#define buf_trunc(v, n) \
    ((v) = buf_grow1((v), sizeof(*(v)), n - buf_capacity(v), BUF_ABORT))

#define buf_clear(v) \
    ((v) ? (buf_ptr((v))->size = 0) : 0)

void *buf_grow1(void *v, size_t esize, ptrdiff_t n, buf_abort_t buf_abort);